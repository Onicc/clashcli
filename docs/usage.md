...
# 固定版本（也可用于手动降级 CLI）：
curl -q -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/Onicc/clashcli/main/install.sh | sh -s -- --version v0.1.3
```
...
ASN 使用 MaxMind 创建的 GeoLite 数据，适用独立的数据许可及更新要求，详见[第三方声明](../THIRD_PARTY_NOTICES.md)。
