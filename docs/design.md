...
| 功能 | 持久状态与生效方式 | 验收重点 |
|---|---|---|
| 初始化 | 隐藏输入 URL，下载安装组件，登记活动订阅，启用 systemd | 中途失败后继续、保留配置重装 |
| 系统代理 | 保存用户原值，再写入 GNOME/KDE 与登录环境 | 新登录与 GIO 实际请求、关闭恢复、外部修改保护 |
| TUN | 作为本机覆盖项写入新一代配置，经事务重载 | IPv4/IPv6、TCP/UDP、DNS、权限不足回滚 |
| 订阅管理 | 命名登记、一个活动源，其余只维护候选版本 | 添加、修改、切换、删除、节点选择 |
| 定时更新 | 每源独立日历 timer，持久补执行与随机延迟 | 真实 timer 触发，不用 mock 代替 |
| 原子更新 | 订阅与所有 provider 组成不可变快照 | 错误输入、并发、重载失败、事务中断 |
| 状态与日志 | 区分保存偏好和实际运行状态；JSON 不输出凭据 | 脱敏、错误状态、无交互可脚本调用 |
| 面板与卸载 | 静态 MetaCubeXD；停机恢复代理，按需清除私有数据 | API 认证、浏览器操作、SSH、资源残留 |

系统代理不等同于强制接管所有程序：GNOME/KDE 客户端须遵循系统设置，服务器的后续登录会话继承环境配置，独立 systemd 服务仍由各自 unit 决定环境。TUN 与系统代理可以分别开关，不互相隐式修改。

## 模块职责

`store.go` 管理私有目录、登记、原子文件写入和锁；`download.go` 管理 TLS、下载边界、摘要与组件安装；`config.go` 负责识别、规范化和快照；`core.go` 封装预检进程与 API；`transaction.go` 负责提交、恢复与历史；`proxy.go` 管理系统代理所有权；`service.go` 管理 unit 与 timer；`cli.go` 提供命令和交互。

内核接口可替换为测试实现以注入失败，但 Linux 验收不替换内核、systemctl、桌面配置工具或流量链路。
...
历史保留按订阅原文和依赖内容去重：每源保留最近两个不同内容版本及被引用的活动版本。TUN 开关产生的运行配置代次不会挤掉前一个订阅内容版本。失败候选不进入历史，删除非活动订阅后会清理其已提交历史。

### 故障处理

| 失败位置 | 处理 |
|---|---|
| 下载、TLS、格式或依赖预检 | 删除未提交候选，旧活动状态不变 |
| 获取锁后发现修订号变化 | 不覆盖并发修改，返回重试提示 |
| 原子指针切换后、提交前退出 | 留下事务记录，恢复旧指针和登记 |
| API 重载或实际状态检查失败 | 恢复旧版本；必要时释放锁后重启内核 |
| 提交标记落盘后退出 | 恢复并确认新版本，完成事务 |
| 外部进程修改系统代理 | 只还原仍属于 clashcli 的值，保留外部变更 |
| systemd 内核进程异常退出 | 停止辅助命令撤销系统代理，限次重启并恢复偏好 |

更新下载不长期占用全局生效锁；每个订阅的下载锁防止重复更新同一源。网络错误与 HTTP 5xx/429 最多重试三次，4xx 不盲目重试；单个订阅/依赖限制 16 MiB，下载和预检均可取消。所有自动任务失败返回非零，由 journald 保留结果供状态命令诊断。
